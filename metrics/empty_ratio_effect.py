...
empty_ratio_values = np.linspace(0.1, 0.9, 50)  # Empty ratio values to test
final_homogeneity_values = []
final_morans_i_values = []

def simulate_for_empty_ratio(empty_ratio):
    """Run the simulation for a specific empty ratio without animation."""
    schelling = Schelling(n_agent_classes=2, tolerance_threshold=0.5, lattice_m=50, lattice_n=50, empty_ratio=empty_ratio, seed=0)
...
for empty_ratio in empty_ratio_values:
    print(f"Simulating for empty ratio: {empty_ratio:.2f}")
    
    final_homogeneity, final_morans_i = simulate_for_empty_ratio(empty_ratio)
...
plt.subplot(1, 2, 1)
plt.plot(empty_ratio_values, final_homogeneity_values, marker='o')
plt.xlabel('Empty Ratio')
plt.ylabel('Final Homogeneity')
plt.title('Impact of Empty Ratio on Homogeneity')

plt.subplot(1, 2, 2)
plt.plot(empty_ratio_values, final_morans_i_values, marker='o')
plt.xlabel('Empty Ratio')
plt.ylabel("Final Moran's I")
plt.title('Impact of Empty Ratio on Moran\'s I')
